...
export class TextileDashboard extends Component {
    ...
    async loadData() {
        ...
        try {
            // Call the python model directly using the ORM service with filters
            const data = await this.orm.call("textile.dashboard", "get_dashboard_data", [], {
                filters: { period: this.state.currentPeriod }
            });
    ...
    async onPeriodChange(ev) {
        this.state.currentPeriod = ev.target.value;
        await this.loadData();
    }
    ...
    openWipReport() {
        this.openAction('textile.wip.inventory.report', [], 'WIP Inventory Report', [[false, 'list'], [false, 'pivot']]);
    }
    ...
    openQualityAction(type) {
        ...
        if (type === 'pending') {
            domain = [['state', '=', 'draft']];
            title = 'Pending Quality Batches';
        } else if (type === 'passed') {
            domain = [['state', '=', 'done']];
            title = 'Completed Quality Batches';
        } else if (type === 'failed') {
            domain = [['state', '=', 'done'], ['failed_qty', '>', 0]];
            title = 'Quality Batches with Failed Units';
        } else if (type === 'rework') {
            domain = [['state', '=', 'done'], ['rework_qty', '>', 0]];
            title = 'Rework Batches';
        } else if (type === 'scrap') {
            domain = [['state', '=', 'done'], ['scrap_qty', '>', 0]];
            title = 'Scrap Batches';
        }
    ...
    openWorkcenterOrders(workcenterId, state) {
        let domain = [['workcenter_id', '=', workcenterId]];
        let title = 'Workcenter Orders';
        
        if (state === 'waiting') {
            domain.push(['state', 'in', ['waiting', 'pending']]);
            title = 'Pending Work Orders';
        } else if (state === 'ready') {
            domain.push(['state', '=', 'ready']);
            title = 'Ready Work Orders';
        } else if (state === 'progress') {
            domain.push(['state', '=', 'progress']);
            title = 'In Progress Work Orders';
        } else {
            domain.push(['state', 'in', ['waiting', 'pending', 'ready', 'progress']]);
