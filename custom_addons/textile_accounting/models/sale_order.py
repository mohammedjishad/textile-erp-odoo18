    ...
    def action_confirm(self):
        ...
        for order in self:
            for line in order.order_line:
                ...
                line.write({'purchase_price': line.purchase_price})
            order.order_line._compute_line_cost()
            order.order_line._compute_line_margin()
            order._compute_sales_profitability()
        return res
    ...
    def _compute_purchase_price(self):
        for line in self:
            ...
            if mos:
                ...
                unit_cost = (total_mo_cost / total_mo_qty) if total_mo_qty > 0 and total_mo_cost > 0 else line.product_id._get_product_cost()
            else:
